<template>
    ...
    <div class="delete">
        <ValidateCard @deleted="load()" :remove=true v-for="user in needToRemoveList" :key="user.user_id" :name="user.name" :email="user.email" :role="user.role" :userId="user.user_id"></ValidateCard>
    </div>
    <FloatingTagCreateButton></FloatingTagCreateButton>
</template>

<script>
import ValidateCard from '../components/ValidateCard.vue';
import FloatingTagCreateButton from '../components/FloatingTagCreateButton.vue';
import {show_error_and_logout} from '../utils/index';
import {USER_API} from '../assets/common';

export default {
    components: {ValidateCard, FloatingTagCreateButton},
    data () {
        return {
    ...
    created(){
        this.$store.dispatch("set_active_tab", "manage");
    },
